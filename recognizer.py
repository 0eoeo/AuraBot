...
async def recognize(request: Request, background_tasks: BackgroundTasks):
    ...
    if not (any(word in lower_text for word in trigger_words) or "?" in lower_text):
        print("🔎 Ни обращения, ни вопроса не найдено. Ответ не требуется.")
        return '', 204
